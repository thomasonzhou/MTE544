...
class PID_ctrl:
    ...
    def __update(self, stamped_error):
        ...
        for i in range(1, len(self.history)):
            ...
            # error_dot+= ... 
                        # If dt is zero (timestamps are identical), avoid division by zero
            if dt == 0:
                continue
            
            # Calculate the difference in error
            error_prev = self.history[i-1][0]
            error_curr = self.history[i][0]

            # Calculate the error derivative for this segment
            error_dot += (error_curr - error_prev) / dt
